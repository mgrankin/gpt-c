...
def build_default_clip(seq_length:int = None):
    clip = TextCLIP( 
                    embed_dim = 512, 
                    context_length = 75, 
                    vocab_size = 49408,
...
def load(fname, seq_length=None):
    params = pickle.load(open(fname, 'rb'))
    
    def raw_encode_text(tokens):
        return build_default_clip(seq_length).encode_text(tokens)

    hk_encode_text = hk.without_apply_rng(hk.transform(raw_encode_text)).apply
    encode_text = partial(hk_encode_text, params)

    def raw_encode(tokens):
        return build_default_clip(seq_length).encode(tokens)

    hk_encode = hk.without_apply_rng(hk.transform(raw_encode)).apply
    encode = partial(hk_encode, params)
    
    return encode, encode_text